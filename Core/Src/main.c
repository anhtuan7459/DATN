...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  MX_Queue_tx_Config();
  HAL_DMAEx_List_LinkQ(&handle_GPDMA1_Channel5, &Queue_tx);
  __HAL_LINKDMA(&hspi1, hdmatx, handle_GPDMA1_Channel5);
  MX_Queue_rx_Config();
  HAL_DMAEx_List_LinkQ(&handle_GPDMA1_Channel4, &Queue_rx);
  __HAL_LINKDMA(&hspi1, hdmarx, handle_GPDMA1_Channel4);
  BL0940_Init();

//  int direction = BL0940_CheckSensorDirection();
//
//  while (direction == -1)
//  {
//      HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_2);
//      HAL_Delay(2000);
//      direction = BL0940_CheckSensorDirection();
//  }
//  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_2, GPIO_PIN_RESET);  // Tắt LED
  /* USER CODE END 2 */
...
static void MX_RTC_Init(void)
{
  ...
  */
  ...
  {
      // 👉 Chỉ set thời gian khi chưa khởi tạo
      sTime.Hours = 0x16;
      sTime.Minutes = 0x05;
      sTime.Seconds = 0x00;
      sTime.DayLightSaving = RTC_DAYLIGHTSAVING_NONE;
